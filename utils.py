...
class VLLMBot(Bot):
    def __init__(self, key_path="", patience=3, model="Qwen/Qwen2.5-VL-32B-Instruct") -> None:
        ...
        import threading
        self.llm = LLM(model=model, trust_remote_code=True, max_model_len=12000, tensor_parallel_size=4, disable_custom_all_reduce=True, gpu_memory_utilization=0.8, enforce_eager=True)
        self.name = "vllm"
        self.model = model
